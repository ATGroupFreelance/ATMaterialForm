...
const UploadImageButton = ({ _formProps_, label, onChange, value, disabled, accept, error, helperText, authToken }) => {
    ...
    useEffect(() => {
        if (value) {
            if (getFile)
                getFile(value, authToken, 128, 128)
                    .then(res => {
    ...
    return <div style={{ flexDirection: "column", alignItems: "center", textAlign: 'center' }}>
        ...
        <Button component="label" sx={{ width: '128px', height: '128px' }} variant={"outlined"} disabled={disabled || loading} color={error ? 'error' : 'primary'}>
